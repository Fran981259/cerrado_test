...
def test_3_filter(articles):
    ...
    try:
        from app.filter import check_sensitive, filter_articles
...
Escreva de forma clara e envolvente."""
            ...
            result = client.rewrite_article(article, reporter_prompt, "Por Enzo Bianchi, do Portal Cerrado")

            if result.get("rewritten_content"):
                print("✅ Reescrito com sucesso!\n")
                content = result["rewritten_content"]
                # Mostra primeiros 400 chars
                print("Conteúdo (primeiros 400 chars):")
                print(f"{'─' * 50}")
                print(content[:400] + "...")
                print(f"{'─' * 50}")
                return result
...
def test_7_full_pipeline():
    ...
    if rewritten and rewritten.get("rewritten_content"):
        ...
        print(f"{'=' * 60}")
        print("Uma matéria foi: COLETADA → CLASSIFICADA → FILTRADA → REESCRITA")
        return True
    else:
        print(f"\n{'=' * 60}")
