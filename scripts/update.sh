...
required_vars=(POSTGRES_USER POSTGRES_PASSWORD REDIS_PASSWORD PUBLISH_API_KEY CORS_ALLOWED_ORIGINS NEXT_PUBLIC_SITE_URL FLOWER_USER FLOWER_PASSWORD)
...
if [ "$(docker info --format '{{.Swarm.LocalNodeState}}' 2>/dev/null || echo inactive)" = "active" ]; then
  ...
  docker build -t "$BACKEND_IMAGE_RELEASE" .
  ...
  BACKEND_IMAGE="$BACKEND_IMAGE_RELEASE" FRONTEND_IMAGE="$FRONTEND_IMAGE_RELEASE" docker stack deploy -c docker-stack.swarm.yml "$STACK_NAME" --with-registry-auth
