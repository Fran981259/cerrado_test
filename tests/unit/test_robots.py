...
from app.robots import _fetch_and_parse, is_allowed
...
def test_robots_fetch_and_parse_denies_on_failure():
    # Falhas ao obter robots.txt não autorizam scraping sem uma política verificável.
    with patch("app.robots._fetch_raw", side_effect=Exception("network fail")):
...
def test_miner_blocks_robots():
    ...
    with patch("app.robots.is_allowed", return_value=False):
        res = miner._mine_portal(
            {"name": "Test", "rss": "https://example.com/rss", "url": "https://example.com"}, "technology", limit=5
        )
        assert res == []
