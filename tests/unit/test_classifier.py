...
def test_high_importance_tech():
    c = NewsClassifier()
    art = {
        "title": "OpenAI announces GPT-5 with breakthrough",
        "summary": "OpenAI revela novo modelo",
        "category": "technology",
        "source": "TechCrunch",
    }
...
def test_low_importance_gossip_rejected():
    c = NewsClassifier()
    art = {
        "title": "Celebrity shares recipe for chocolate cake lifestyle gossip",
        "summary": "horoscope viral video meme",
        "category": "culture",
        "source": "Blog",
    }
    res = c.classify(dict(art))
...
def test_engagement_viral_scandal():
    c = NewsClassifier()
    art = {
        "title": "Shocking scandal exposed: arrested after leaked video",
        "summary": "unbelievable controversy",
        "category": "general",
        "source": "Portal",
    }
    res = c.classify(dict(art))
