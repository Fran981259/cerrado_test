...
def post_top_news_twitter():
    ...
    try:
        ...
        posted_subquery = db.query(PublicationLog.article_id).filter(
            PublicationLog.action == "social_twitter"
        ).subquery()
        
        # Find the top article (by final_score or importance_score) published recently
        top_article = db.query(NewsArticle).filter(
